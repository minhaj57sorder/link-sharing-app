    ...
    <div class="flex -m-1 ">
                    ...
                    <div class="">
                      <div class="flex justify-center mb-2">
                        <div class="border-4 border-violet-600 rounded-full">
                          <img v-if="formDataLists.profilePicture"
                            class="w-[5rem] h-[5rem] rounded-full overflow-hidden" :src="tempPictureUrl" alt="" />
                          <img v-else-if="formDataListsPrev.profilePicture == '/images/upload-placeholder.jpg'"
                            class="w-[5rem] h-[5rem] rounded-full overflow-hidden" src="/images/user-placeholder.jpg"
                            alt="" />
                          <img v-else class="w-[5rem] h-[5rem] rounded-full overflow-hidden"
      ...
      <div class="md:w-3/5 w-full p-1">
        ...
        <q-card class="bg-white mb-2" flat>
          <q-card-section>
            <div>
              <h6 class="text-h6">Profile Details</h6>
              <p class="text-p">Add details to create personal touch to your profile.</p>
            </div>
            ...
            <div>
              ...
              <div class="flex justify-between mb-6">
                ...
                <div class="md:w-1/3 w-full flex justify-center ">
                  <div class="relative hover-effect-image-upload">

                    <img v-if="formDataLists.profilePicture" class="w-[10rem]" :src="tempPictureUrl" alt="" />
                    <img v-else class="w-[10rem]" :src="backendUrlLink + '/' + formDataListsPrev.profilePicture"
                      alt="" />
                    <div class="image-overlap absolute z-1 left-0 top-0 w-full h-full flex justify-center items-center">
                      <span class="text-white">Upload</span>
                    </div>
                    <q-file class="absolute z-10 left-0 top-0 opacity-0 w-full h-full" outlined dense
                      v-model="formDataLists.profilePicture" label="Upload Profile picture file "
                      v-on:update:model-value="createTempPic">
                ...
                <div class="md:w-1/3 w-full  flex items-center p-2">
                  <span>Profile Picture must be 1:1</span>
                </div>
              </div>
              <div class="mb-6">
                <q-input label="First name " type="text" placeholder="First name" v-model="formDataLists.firstName"
                  outlined dense />
              </div>
              <div class="mb-6">
                <q-input label="Last name " type="text" placeholder="Last name" v-model="formDataLists.lastName"
                  outlined dense />
              </div>
              <div class="mb-6">
                <q-input label="User name " type="text" placeholder="User name" v-model="formDataLists.userName"
              ...
              <div class="mb-6">
                <q-input label="Email *" type="email" placeholder="name@example.com" v-model="formDataLists.email"
                  outlined dense />
        ...
        <q-card class="bg-white" flat>
          <q-card-section class="flex justify-between">
            <div class="q-pa-none border-bottom-1" v-if="userAuthStore?.userData">
              <q-btn flat class="full-width text-weight-bold bg-red text-white" align="left" no-caps icon="logout"
                @click="userAuthStore.signOut">
                <span style="padding-left: 8px">Sign out</span>
              </q-btn>
            </div>
            <div v-else></div>
            <q-btn label="Save" class="bg-primary text-white" @click="updateUser" no-caps />
          </q-card-section>
        </q-card>
      </div>
    </div>
