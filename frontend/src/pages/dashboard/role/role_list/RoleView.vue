    ...
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createRole')">
        <q-btn
          ...
          label="New Role"
          to="/admin/role/role_list/create"
          no-caps
        />
      </template>
    </div>
...
const onEdit = (e) => {
  router.push(`/admin/role/role_list/${e._id}`);
};
const onDelete = (e) => {
  ...
  try {
    ...
    for (
      ...
      for (
        ...
        const existOrNot = roleList.value.roles[roleIndex].permissions.find(e=>e._id === response.data.permissions[permissionIndex]._id) ? true: false;
    ...
    permissionIdRoleIdArray.value = response.data.permissions.map((e) => e._id);
