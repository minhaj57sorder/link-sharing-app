<template>
  <q-page padding>
    <div class="w-full flex justify-end pb-3">
      <template v-if="userAuthStore.checkShowOnPermission('createUserLinks')">
        <q-btn style="background: green; color: white" label="New User links"
          to="/admin/user_links/user_link_list/create" />
      </template>
    </div>
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <CompactQTable :columns="columns" :data-list="userLinksList?.userLinks || []" :page="userLinksList?.page || 1"
        :pages="userLinksList?.pages || 1" @updatePaginate="updatePaginate" @onEdit="onEdit" @onDelete="onDelete"
        @onFilter="onFilter" :showEdit="userAuthStore.checkShowOnPermission('editUserLinks')"
        :showDelete="userAuthStore.checkShowOnPermission('deleteUserLinks')" />
    ...
    <q-dialog v-model="deletePopup">
      <q-card style="width: 300px; max-width: 80vw">
        ...
        <q-card-section class="q-pt-none">
          Are you sure you want to delete this userLinks?
        </q-card-section>
        <q-card-actions align="right" class="bg-white text-teal">
          <q-btn flat label="Cancel" v-close-popup />
          <q-btn flat label="Delete" v-close-popup @click="deleteUserLinks" color="red" />
        </q-card-actions>
      </q-card>
    </q-dialog>
  </q-page>
...
const onEdit = (e) => {
  router.push(`/admin/user_links/user_link_list/${e._id}`);
};
const onDelete = (e) => {
...
const getUserLinksList = async () => {
  ...
  const config = {
    method: 'GET',
    url: 'api/user_links/user_link_lists',
    headers: {
  ...
  try {
    ...
    const datalist = {
      ...
      userLinks: response.data.userLinks.map(e => {
        const links = e.links.map(x => x.linkType)
        return {
          ...e,
          links: links.join(", ")
        }
...
watch(paginationAndFilter, debounce(getUserLinksList, 1000));
const deleteUserLinks = async () => {
  // userAuthStore.checkLogin()
  const config = {
    method: 'delete',
    url: 'api/user_links/user_link_lists/' + deleteId.value,
    headers: {
      Authorization: `Bearer ${getToken('token')}`,
    },
  };
  try {
    ...
    const userLinks = userLinksList?.value.userLinks.filter(
      (item) => item._id !== deleteId.value,
    );
    userLinksList.value = {
      ...JSON.parse(JSON.stringify(userLinksList?.value)),
      userLinks,
    };
    if (
      userLinksList?.value.userLinks.length < 5 &&
      userLinksList?.value.pages !== 1
    ) {
      getUserLinksList();
    }
    $q.loading.hide();
